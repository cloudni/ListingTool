...
class EBayListingController extends Controller
{
    ...
    public function actionSearchAppliedListings()
    {
        ...
        /*$params['searchKeyword'] = ' RED BTG';//(string)$_POST['searchKeyword'];
        $params['searchSite'] = '100';//(string)$_POST['searchSite'];
        $params['searchCategory'] = '50073';//(string)$_POST['searchCategory'];
        $params['searchStore'] = '1';//(string)$_POST['searchStore'];
        $params['searchMode'] = 'normal';//(string)$_POST['searchMode'];
        $params['searchEngine'] = 'norma';//(string)$_POST['searchEngine'];*/
        ...
        {
            $result = array('status'=>'success', 'data'=>"Error Code: ".$ex->getCode().", ".$ex->getMessage());
            echo json_encode($result);
        }
    }
    ...
    public function actionBulkUpdateSubmit()
    {
        ...
        if(count($params['applied_listings'])>3)
        {
            ...
            {
                Yii::app()->user->setFlash('Success', 'Request has been added into schedule successfully. You will receive notification after done!<br />If you want to execute immediately, please select less than or equal to 3 items.');
                $this->redirect($this->createAbsoluteUrl("eBay/eBayListing/bulkUpdate", array()));
            }
            else
            {
